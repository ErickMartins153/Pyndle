...
def checar(
    nomeUsuario, senha
):  # Essa função checa todas as restrições de nome de usuario que a gente decidir botar
    quantidadeUsuario = sgbd.execute(
        "SELECT COUNT(login) FROM usuarios WHERE login = ?", (nomeUsuario,)
    )
...
def logarUsuario(nomeUsuario: str, senha: str):
    ...
    count = quantidadeUsuario.fetchone()[
        0
    ]  # essa variavel tá basicamente perguntando se o nomeUsuario tá cadastrado no sistema, pq se não tiver, já avisa que o login foi digitado errado ou que você não foi cadastrado ainda
